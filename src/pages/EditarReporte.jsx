...
export default function EditarReporte() {
  ...
  const loadReporte = async () => {
    try {
      setLoading(true);
      const reporte = await reportService.getReportById(id);
      
      // Parsear campos JSON
      const fotografias = typeof reporte.fotografias === 'string' ? 
        JSON.parse(reporte.fotografias) : reporte.fotografias || {};
      const actividades = typeof reporte.actividades === 'string' ? 
        JSON.parse(reporte.actividades) : reporte.actividades || [];
      const materiales = typeof reporte.materiales === 'string' ? 
        JSON.parse(reporte.materiales) : reporte.materiales || [];

      // Convertir snake_case a camelCase para el formulario
      const formattedData = {
        tipoMantenimiento: reporte.tipo_mantenimiento || '',
        modeloUTR: reporte.modelo_utr || '',
        fechaMantenimiento: reporte.fecha_mantenimiento || '',
        horaInicio: reporte.hora_inicio || '',
        horaTermino: reporte.hora_termino || '',
        ...
        radioGabinete: reporte.radio_gabinete || '',
        potenciaSalida: reporte.potencia_salida || '',
        rssi: reporte.rssi || '',
        umbralRecepcion: reporte.umbral_recepcion || '',
        frecuenciaMhz: reporte.frecuencia_mhz || '',
        rx: reporte.rx || '',
        tx: reporte.tx || '',
        cablePigtail: reporte.cable_pigtail || '',
        supresor: reporte.supresor || '',
        cableLT: reporte.cable_lt || '',
        alturaAntena: reporte.altura_antena || '',
        repetidorEnlace: reporte.repetidor_enlace || '',
        canalUCM: reporte.canal_ucm || '',
        actividades: actividades,
        potenciaRadio: reporte.potencia_radio || '',
        potenciaIncidente: reporte.potencia_incidente || '',
        potenciaReflejada: reporte.potencia_reflejada || '',
        vswr: reporte.vswr || '',
        voltajeAcometida: reporte.voltaje_acometida || '',
        resistenciaTierra: reporte.resistencia_tierra || '',
        voltajeFuente: reporte.voltaje_fuente || '',
        resistenciaBateria: reporte.resistencia_bateria || '',
        porcentajeBateria: reporte.porcentaje_bateria || '',
        anguloAzimut: reporte.angulo_azimut || '',
        materiales: materiales,
        calibreBajante: reporte.calibre_bajante || '',
        observaciones: reporte.observaciones || '',
        fotografias: fotografias,
        codigoRadio: reporte.codigo_radio || ''
      };
      ...
      if (fotografias && Object.keys(fotografias).length > 0) {
        ...
        Object.keys(fotografias).forEach(fieldName => {
          if (Array.isArray(fotografias[fieldName]) && fotografias[fieldName].length > 0) {
            imagesState[fieldName] = fotografias[fieldName].map(path => {
              // Construir URL correcta - las imágenes están en /reports/temp/
              const imageUrl = path.startsWith('http') 
                ? path 
                : `${baseURL}/${path}`;
              
              console.log(`🖼️ Imagen ${fieldName}:`, imageUrl);
        ...
        console.log('✅ Estado de imágenes cargado:', imagesState);
  ...
  const handleSubmit = async () => {
    try {
      console.log('📤 Actualizando reporte con datos:', formData);
      ...
      {loading ? (
        <div className="min-h-screen bg-[#F5F5F5] flex items-center justify-center">
          <div className="text-center">
            <div className="animate-spin rounded-full h-16 w-16 border-b-4 border-[#00A859] mx-auto mb-4"></div>
            <p className="text-xl text-gray-600">Cargando reporte...</p>
          </div>
        </div>
      ) : (
      ...
      <main className="flex-1 lg:ml-0 mt-20 lg:mt-0 p-4 lg:p-8">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold text-[#00A859] mb-2">Editar Reporte</h1>
            <p className="text-gray-600">Modifique los datos del reporte de mantenimiento</p>
          </div>
                          ...
                          {uploadedImages[foto.name].map((img, imgIndex) => (
                            <div key={imgIndex} className="relative group">
                              <img
                                ...
                                onError={(e) => {
                                  console.error('Error cargando imagen:', img.preview);
